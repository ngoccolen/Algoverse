...
const ActivityBarChart = ({ data }) => {
    ...
    return (
        <div className="flex items-end justify-between h-40 gap-2 mt-4 px-4">
            {data.map((item, index) => {
                const heightPercent = item.count === 0 ? 5 : (item.count / maxCount) * 100;
                return (
                    <div key={index} className="flex flex-col items-center flex-1 group relative">
                        <div className="absolute -top-8 opacity-0 group-hover:opacity-100 transition bg-slate-800 text-white text-xs px-2 py-1 rounded mb-1 whitespace-nowrap z-10">
                            {item.count} bài
                        </div>
                        <div className={`w-full max-w-[40px] rounded-t-lg transition-all duration-500 ease-out ${item.count > 0 ? 'bg-blue-500 group-hover:bg-blue-600' : 'bg-slate-100'}`} style={{ height: `${heightPercent}%` }}></div>
                        <span className="text-xs text-slate-500 mt-2 font-medium">{item.dayName}</span>
                    </div>
                );
            })}
        </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100 relative">
            <div className="h-32 bg-gradient-to-r from-blue-600 to-purple-600 relative">
               <button onClick={handleLogout} className="absolute top-4 right-4 bg-white/20 hover:bg-white/40 p-2 rounded-full text-white transition backdrop-blur-sm"><LogOut size={18}/></button>
            </div>
            ...
            <div className="px-6 pb-8 relative text-center -mt-16">
              ...
              <div className="mt-6 text-left space-y-3 bg-slate-50 p-4 rounded-xl border border-slate-100">
                 {user?.bio && <p className="text-slate-600 text-sm italic">"{user.bio}"</p>}
          ...
          <div className="bg-white rounded-3xl shadow-xl p-6 border border-slate-100">
             ...
             <div className="grid grid-cols-2 gap-4 text-center mb-6">
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                    <div className="text-2xl font-black text-blue-600">{user?.totalSubmissions || 0}</div>
                    <div className="text-xs font-bold text-slate-400 uppercase">Bài nộp</div>
                </div>
                <div className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                    <div className="text-2xl font-black text-green-600">{user?.stats?.Total || 0}</div>
                    <div className="text-xs font-bold text-slate-400 uppercase">Đã giải</div>
                </div>
             </div>
                    ...
                    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
                       ...
                       <div className="bg-slate-50 rounded-2xl p-6 border border-slate-200">
                          <h3 className="font-bold text-slate-800 mb-2 flex items-center gap-2">
                              <Activity className="text-green-500"/> Hoạt động 7 ngày qua
                          </h3>
